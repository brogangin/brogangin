...
const Team = ({ team, isToday, score }: TeamProps) => {
  return (
    <div className="flex flex-col w-1/2 items-center justify-center">
      ...
      <img
        ...
        className={clsx("w-[100px] h-[100px] border-[4px] border-black shadow-[5px_5px_0_0_#000000] rounded-[14px]", isToday && "w-[180px] h-[120px] border-[5px] shadow-[10px_10px_0_0_#000000]")}
      />
      <p className="font-bold text-lg mt-2 text-center">{team?.name}</p>
      <p className="font-bold text-2xl text-center">{score !== null ? String(score) : ""}</p>
    </div>
  );
};
...
export default function Card({ match, isToday }: CardProps) {
  ...
  return (
    ...
    >
      <Team team={HomeTeam} score={match?.HomeTeamScore ? match?.HomeTeamScore : 0} isToday={isToday} />
      <Versus date={match?.DateUtc} />
      <Team team={AwayTeam} score={match?.AwayTeamScore ? match?.AwayTeamScore : 0} isToday={isToday} />
    </div>
  );
}
